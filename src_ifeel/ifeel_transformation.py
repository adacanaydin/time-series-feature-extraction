...
def feature_transformation(df_test, alphabet_size, time_business_start, time_business_end):
    ...
    for i in np.arange(0, df_test.shape[0]):
        ...
        # ADJUSTMENTS MADE by Ada Canaydin
        ###df_znorm_all_house = df_znorm_all_house.append(y['normalized_power'], ignore_index=True)
        df_znorm_all_house =pd.concat([df_znorm_all_house, y['normalized_power']], axis=1, ignore_index=True)
        ###df_SAX_number_all_house = df_SAX_number_all_house.append(y['level'], ignore_index = True)
        df_SAX_number_all_house =pd.concat([df_SAX_number_all_house, y['level']], axis=1, ignore_index=True)
        ###df_SAX_all_house = df_SAX_all_house.append(y['letter'], ignore_index = True)
        df_SAX_all_house =pd.concat([df_SAX_all_house, y['letter']], axis=1, ignore_index=True)
    ###newly added:
    df_znorm_all_house = df_znorm_all_house.transpose()
    df_SAX_number_all_house = df_SAX_number_all_house.transpose()
    df_SAX_all_house = df_SAX_all_house.transpose() 
    ###
